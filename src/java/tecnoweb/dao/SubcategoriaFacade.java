...
public class SubcategoriaFacade extends AbstractFacade<Subcategoria> {
    ...
    public List<Subcategoria> findByCategoria(Integer codCat){
        Query q;

        q = this.getEntityManager().createQuery("SELECT s FROM Subcategoria s WHERE s.idCategoria.idCategoria = :codCat");
