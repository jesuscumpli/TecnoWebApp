...
<html>
    ...
    <%
        ...
        List<SubcategoriaDTO> subcategorias = (List<SubcategoriaDTO>)request.getAttribute("subcategorias");
    ...
    <div class="row">
        ...
        <div class="row">
            <small class="w-100 text-muted">Introduce aquí una nueva categoría</small>
            <form method="post" accept-charset="UTF-8" action="AnadirCategoria">
                <input type="text" name="nombre">
                <input class="text-white bg-success" type="submit" value="Añadir nueva categoría"/>
            </form>
        </div>
        ...
        %>
                <tr>
                    ...
                    <%if(!cat.getNombreCategoria().equals("Por definir")){%> <td><a class="btn btn-danger" href="BorrarCategoria?idCategoria=<%= cat.getIdCategoria() %>">Borrar</a></td> <%}%>
                    <%if(!cat.getNombreCategoria().equals("Por definir")){%> <td><button class="btn btn-secondary" onclick="editarCategoria(<%=cat.getIdCategoria()%>)" >Editar</button></td> <%}%>
                    <%if(!cat.getNombreCategoria().equals("Por definir")){%><td><a class="btn btn-info" href="ListadoConsultarSubcategorias?idCategoria=<%= cat.getIdCategoria() %>">Consultar subcategorías</a></td><%}%>
                </tr>          
        <%
                }
        %>    
        ...
        <div id="subcategorias" style="margin-left:auto">
            ...
            <div class="row">
                <form method="post" accept-charset="UTF-8" action="AnadirSubcategoria?idCategoria=<%= idCategoria%>">
                    <small class="text-muted">Introduce aquí una nueva subcategoría</small>
                    <br>
                    <input type="text" name="nombre"></input>
                    <input class="text-white bg-success" type="submit" value="Añadir nueva subcategoría"/>
                </form>
            </div>
            ...
            <%
                    for(SubcategoriaDTO subcat: subcategorias){
            %>
                    <tr>
                        ...
                        <td><a class="btn btn-danger" href="BorrarSubcategoria?idSubcategoria=<%= subcat.getIdSubcategoria() %>">Borrar</a></td>   
                        <td><button class="btn btn-secondary" onclick="editarSubcategoria(<%=subcat.getIdCategoria().getIdCategoria()%>,<%=subcat.getIdSubcategoria()%>)">Editar</button></td>
                        </tr>         
            <%
                    }
            %>    
